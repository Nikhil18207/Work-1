...
def verify_integration():
    ...
    print("3️⃣  Checking unstructured data...")
    unstructured_dirs = [
    ...
    print("7️⃣  Testing enhanced rule engine...")
    try:
        from backend.engines.enhanced_rule_engine import EnhancedRuleEngine
        ...
        print(f"   ⚠️  Enhanced rule engine: {e}")
    
    print()
    
    # 8. Test scenario detector
    print("8️⃣  Testing scenario detector...")
    try:
    ...
    print("9️⃣  Checking vector database...")
    ...
    if Path("data/vector_db").exists() and len(list(Path("data/vector_db").glob("*"))) > 1:
        ...
        print("   ⚠️  RAG Engine ↔ Vector Store (not initialized)")
